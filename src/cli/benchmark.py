...
class APIBenchmark:
        ...
        async with aiohttp.ClientSession(
            ...
            with Progress() as progress:
                task_id = progress.add_task(
                    f"Benchmarking {method} {endpoint}",
                    total=requests
                )
    ...
    def _display_result(self, result: BenchmarkResult):
    ...
    def display_summary(self, results: List[BenchmarkResult]):
        ...
        for result in results:
            ...
            if result.requests_per_second < 100:  # Below minimum threshold
                status = "❌"
            elif result.p95_response_time > 200:  # Above 200ms target
                status = "⚠️"
            elif result.error_rate > 1:  # Above 1% error rate
                status = "⚠️"
...
def api(url, token, requests, concurrent, export):
...
def single(url, token, endpoint, method, requests, concurrent, data):
...
def load_test(url, token, target_rps, duration):
    ...
    async def run_load_test():
        ...
        for i in range(duration // chunk_duration):
            ...
            console.print(f"Chunk {i+1}: {result.requests_per_second:.1f} RPS, "
