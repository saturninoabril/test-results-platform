...
from lib.storage import init_storage, get_storage_client, close_storage
...
class StorageManager:
    ...
    async def get_usage_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'bucket': storage_client.settings.storage.bucket_name
            }
    ...
    async def cleanup_orphaned_files(self, dry_run: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            orphaned_size = sum(
                file_info['size'] for file_info in storage_files
    ...
    async def cleanup_old_files(self, older_than_days: int = 30, dry_run: bool = True) -> Dict[str, Any]:
    ...
    async def restore_storage(self, backup_path: Path, restore_data: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'db_records_found': len(db_backup) if db_backup_path.exists() else 0
            }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                health_status['checks']['connection'] = {'status': 'healthy', 'message': 'Storage connection successful'}
            except Exception as e:
            ...
            try:
                ...
                await storage_client.upload_file(test_key, b"health check", "text/plain")
                await storage_client.delete_file(test_key)
                health_status['checks']['write_access'] = {'status': 'healthy', 'message': 'Write access confirmed'}
            except Exception as e:
    ...
    async def monitor_performance(self, test_count: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in track(range(test_count), description="Testing upload performance..."):
                ...
                await storage_client.upload_file(test_key, test_data, "application/octet-stream")
        ...
        try:
            ...
            if format == 'json':
                ...
                if stats['file_types']:
                    ...
                    for ext, info in sorted(stats['file_types'].items(), key=lambda x: x[1]['size'], reverse=True):
...
def cleanup_storage(orphaned, old_files, older_than, dry_run, confirm):
...
def backup_storage(backup_path, metadata_only):
    """Create a backup of storage contents.
    ...
    async def _backup():
        ...
        try:
            ...
            backup_table.add_row("Include Data", "Yes" if result['include_data'] else "No (metadata only)")
...
def restore_storage(backup_path, metadata_only, confirm):
    """Restore storage from backup.
    ...
    async def _restore():
        ...
        try:
            ...
            restore_table.add_row("Include Data", "Yes" if result['restore_data'] else "No (metadata only)")
            restore_table.add_row("Backup Date", result['backup_date'][:19] if result['backup_date'] else 'Unknown')
...
@click.option('--test-count', '-c', type=int, default=10,
              help='Number of test operations to perform')
def monitor_performance(test_count):
    """Monitor storage performance with test operations."""
    async def _monitor():
        manager = StorageManager()
