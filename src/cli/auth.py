...
class TokenManager:
    ...
    async def generate_token(
        ...
        token_info = {
            ...
            "scope": scope.value,
            "permissions": claims.permissions,
            "created_at": claims.metadata["created_at"],
    ...
    def _get_default_permissions(self, scope: TokenScope) -> List[str]:
        ...
        elif scope == TokenScope.READ_ONLY:
            return [
                "frameworks:read", "environments:read",
                "suites:read", "results:read", "artifacts:read"
            ]
        else:
            return []
...
@click.option('--scope', '-s',
              ...
              default='automation', help='Token scope')
@click.option('--expires-days', '-e', type=int, default=365, help='Token expiration in days')
@click.option('--permissions', '-p', multiple=True, help='Specific permissions (can be used multiple times)')
@click.option('--output', '-o', type=click.Choice(['token', 'json', 'table']), default='table',
              help='Output format')
@click.option('--save', is_flag=True, help='Save token metadata locally')
def generate_token(name, scope, expires_days, permissions, output, save):
    """Generate a new automation token.
...
@click.option('--format', '-f', type=click.Choice(['table', 'json']), default='table',
              help='Output format')
def validate_token(token, format):
...
@click.option('--client-secret', prompt=True, hide_input=True, help='GitHub OAuth App Client Secret')
@click.option('--callback-url', help='OAuth callback URL')
def setup_github_oauth(client_id, client_secret, callback_url):
...
def auth_status():
    ...
    # JWT Configuration
    jwt_status = "✅ Configured" if settings.auth.secret_key else "❌ Missing"
...
def troubleshoot():
    ...
    if settings.auth.secret_key:
        ...
        diagnostics.append(("❌", "JWT Secret", "Missing - set AUTH_SECRET_KEY environment variable"))
